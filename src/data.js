  ...
  },
  {
    ...
    problems: [
      ...
      { id: '5-3', title: '성적 상위 k명 뽑기', completed: true, contribution: '검토', percentage: 15, code: '' },
      {
        ...
        title: '계단 오르기',
        completed: true,
        contribution: '문제를 피보나치 구조로 해석해 f(n)=f(n-1)+f(n-2), f(1)=1, f(2)=2 점화식을 도출하고, 재귀 탐색과 반복문 최적화 풀이를 비교함. a, b 두 변수만 갱신하는 O(n) 시간·O(1) 공간 코드를 구현하고 입력값 약 35에서 실행시간 차이를 측정했으며, 황금비 일반항 기반 O(1) 계산법까지 적용해 반복문 결과와 비교하고 큰 입력에서 부동소수점 오차가 생길 수 있음을 분석함. 온라인 채점에서는 실행시간 문구를 제거해야 한다는 출력 형식 문제도 점검함',
        percentage: 100,
        highlight: '심화 학습',
        code: '',
      },
